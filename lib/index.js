...
export function apply(ctx) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (args.window)
                cmdArgs.push('--window', String(args.window));
            if (args.maximize)
                cmdArgs.push('--maximize');
            if (args.region)
                cmdArgs.push('--region', String(args.region));
            if (args.delay)
                cmdArgs.push('--delay', String(args.delay));
    ...
    ctx.tools.register(defineTool({
        name: 'ocr',
        description: '截取屏幕/窗口（可 region/delay），用 OCR 识别其中的文字并**返回文本**。' +
    ...
    ctx.tools.register(defineTool({
        name: 'capture_tabs',
        description: '自动切换并截图浏览器（Chrome/Edge 等 Chromium）的各网页标签，返回每页截图（图片）。' +
        ...
        output: {
            ...
            render: (_args, value) => value.captures.flatMap((c) => {
                const label = `${String(c.title ?? '')}  ${String(c.url ?? '')}`;
                const ref = c.ref;
                return ref
                    ? [{ type: 'text', text: label }, { type: 'image', attachment: ref }]
                    : [{ type: 'text', text: `${label}  (失败: ${String(c.error ?? '')})` }];
            }),
        },
        timeoutMs: 120000,
        async execute(args, exec) {
            ...
            if (args.urls && args.urls.length > 0) {
                ...
                if (args.url)
                    cmdArgs.push('--url-substr', String(args.url));
                if (args.title)
                    cmdArgs.push('--title-substr', String(args.title));
            }
            if (args.full_page)
            ...
            for (const tab of (info.tabs ?? []).slice(0, MAX_TABS)) {
                ...
                try {
                    ...
                    captures.push({ ref: ref, title, url });
                }
                catch (e) {
                    captures.push({ title, url, error: `read/save failed: ${e.message}` });
                }
            }
